...
def video2Masks_instance(filename):
    """
    将视频处理为一帧帧的MASK并保
    ...
    target_classes = segment_frame.select_target_classes(person=True, dog=True)
    ...
    while True:
        ...
        results, output = segment_frame.segmentFrame(frame, segment_target_classes=target_classes,
                                                     show_bboxes=False)
        # 获取MASK
        masks = results['masks']
...
def video2Masks_semantic(filename):
    """
    将视频处理为一帧帧的MASK并保
    ...
    while True:
        ...
        # results, output = segment_image.segmentAsPascalvoc()
        results, output = segment_image.segmentFrameAsPascalvoc(frame)
        # results, output = segment_image.segmentAsPascalvoc(ret,)
        # results, output = segment_frame.segmentFrame(frame, segment_target_classes=target_classes,
        #                                              show_bboxes=False)

        # 获取MASK
        masks = results['masks']
        id = results['class_ids']
        print(id)
        # 构造原图大小的灰度图片，默认黑色
        black_ing = np.zeros(output.shape[:2])
        black_ing = np.where(masks == True, 255, black_ing)
    ...
    def video2Masks_instance(self):
        ...
        while True:
            ...
            if instance_num > 0:
                ...
                for index in range(masks.shape[2]):
                    black_ing = np.where(masks[:, :, index] == True, 255, black_ing)
                output[black_ing == 0] = 0
                # 存储照片
                cv2.imwrite('./mask_img/' + str(frame_index) + '.jpg', black_ing)
                cv2.imwrite('./clip/' + str(frame_index) + '.jpg', output)
...
parser.add_argument("--video", type=str, default='./videos/2.mp4',
                    help="要处理的视频")

parser.add_argument("--mode", type=str, default='mask_i',
...
if mode == 'mask_i':
    vp.video2Masks_instance()

if mode == 'mask_s':
    vp.video2Masks_semantic()
